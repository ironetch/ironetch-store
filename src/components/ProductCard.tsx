...
const ProductCard = ({ title, price, image, href, description, materials }: ProductCardProps) => {
  const [imgError, setImgError] = useState(false);
  const isSlate = materials?.some(m => m.toLowerCase().includes('slate'));
      ...
      <div className="relative aspect-square overflow-hidden bg-slate-900">
        ...
        {showImg ? (
          ...
          <img
            ...
            className="w-full h-full object-contain p-6 transform transition-transform duration-700 group-hover:scale-110"
            style={isSlate ? { filter: 'brightness(0) invert(1)' } : undefined}
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <span className="text-7xl font-black italic text-slate-800 group-hover:text-slate-700 transition-colors">
              {title[0]}
            </span>
          </div>
        )}
